...
export class StateComponent implements OnInit {
  ...
  getAllStates() {
    this.apiService.stateService.getAllState().subscribe(res => {
      this.allStates = res;
      console.log("All State", this.allStates);
  ...
  showModal(type) {
    ...
    if (type == 'new') {
      ...
      if (formControl) {
        this.addStateForm.removeControl('StateID');
      }
    }
    if (type == 'edit') {
      this.isOnEdit = true;
  ...
  onSave() {
    if (!this.isOnEdit) {
      this.apiService.stateService.createState(this.addStateForm.value).subscribe((res: any) => {
        //this.addStateForm.removeControl('StudentNO');
        this.getAllStates();
        this.isVisible = false;
      ...
      console.log("Form Dta", this.addStateForm.value)

      this.apiService.stateService.updateState(this.addStateForm.value).subscribe((res: any) => {
        this.addStateForm.removeControl('StateID');
  ...
  editRecord(id) {
    ...
    this.apiService.stateService.getStateById(id).subscribe((res: any) => {
      ...
      this.addStateForm.addControl('StateID', new FormControl(id));
      this.addStateForm.patchValue(this.singleState)
    })
  }
